...
segregation_analysis <- aggregated_hn %>%
  ...
  left_join(census_data, by = c("MSOA11NM", "MSOA11CD", "Year")) # Perform the join
...
for (nationality in unique(filtered_segregation$Nationality)) {
  ...
  correlation_data <- nat_data %>%
    dplyr::select(
      Dissimilarity_Index, Proportion, Rent, Socialhousing, Owned, Houseprice,
      Overoccupation, Crime, Greenery, PTAL, Lagged_Dissimilarity
    )
...
predictors <- filtered_segregation %>%
  st_drop_geometry() %>%  
  dplyr::select(Proportion, Rent, Socialhousing, Owned, Houseprice,
...
dir.create("scatterplots_fixed", showWarnings = FALSE)

# Specify the variables for scatterplots (on the Y-axis)
y_vars <- c("Rent", "Socialhousing", "Owned", "Houseprice",
            "Overoccupation", "Crime", "Greenery", "PTAL", "Lagged_Dissimilarity")
...
for (nationality in unique(filtered_segregation$Nationality)) {
  ...
  nat_data <- filtered_segregation %>%
    filter(Nationality == nationality) %>%
    mutate(Dissimilarity_Index = as.numeric(Dissimilarity_Index))  # Ensure numeric
  
  # Generate scatterplots for each specified variable
  for (y_var in y_vars) {
    if (y_var %in% colnames(nat_data)) {  # Ensure variable exists in the data
      # Clean the data: remove NA and outliers
      clean_data <- nat_data %>%
        mutate(!!sym(y_var) := as.numeric(!!sym(y_var))) %>%  # Ensure numeric
        filter(
          !is.na(Dissimilarity_Index), !is.na(!!sym(y_var)),  # Remove NA
          Dissimilarity_Index < quantile(Dissimilarity_Index, 0.99, na.rm = TRUE),
          !!sym(y_var) < quantile(!!sym(y_var), 0.99, na.rm = TRUE)
        )
      
      # Create scatterplot
      scatter_plot <- ggplot(clean_data, aes(x = Dissimilarity_Index, y = !!sym(y_var))) +
      ...
      message(paste("Variable", y_var, "not found for nationality:", nationality))
    }
  }
}
```
...
for (nationality in nationalities) {
  ...
  housing_lagged_formula <- paste(y_variable, "~", paste(housing_variables, collapse = " + "), "+ Lagged_Dissimilarity")
