...
final_segregation <- segregation_sf %>%
  ...
  dplyr::select(
    ...
    lagged_diss,
    lagged_Rent,
    lagged_socialhousing = lagged_average_social,
    lagged_Houseprice,
    lagged_Occupation
  ) 
...
# Create a directory for correlation matrix outputs
dir.create("correlation_matrices", showWarnings = FALSE)

# Define housing and neighborhood variables
x_vars <- c("Rent", "Socialhousing", "Ownership", "average_rented", "average_social", 
            "Houseprice", "Overoccupation", "MSOA_crime", "MSOA_percent_green", "PTAL")

# Loop through each nationality to compute and save correlation matrices
...
for (nationality in nationalities) {
  ...
  # Subset variables for correlation
  correlation_data <- nat_data %>% select(Dissimilarity_Index, all_of(x_vars))
  
  # Compute correlation matrix
  corr_matrix <- cor(correlation_data, use = "complete.obs")
  
  # Visualize and save the correlation matrix
  corr_plot <- ggcorrplot(corr_matrix, method = "circle", title = paste("Correlation Matrix for", nationality)) +
    theme_minimal()
...
# Create a directory for scatterplot outputs
...
for (nationality in nationalities) {
  ...
  for (x_var in x_vars) {
    scatter_plot <- ggplot(nat_data, aes_string(x = x_var, y = "Dissimilarity_Index")) +
      geom_point(alpha = 0.5, color = "blue") +
      geom_smooth(method = "lm", se = FALSE, color = "red") +
      labs(title = paste("Scatterplot for", nationality, "-", x_var),
           x = x_var, y = "Dissimilarity Index") +
      theme_minimal()
    
    # Save the scatterplot
    ggsave(filename = paste0("scatterplots/scatterplot_", nationality, "_", x_var, ".png"),
...
for (nationality in nationalities) {
  ...
  for (x_var in x_vars) {
    ...
    ccf_plot <- ccf(nat_data$Dissimilarity_Index, nat_data[[x_var]], lag.max = 10, plot = TRUE,
