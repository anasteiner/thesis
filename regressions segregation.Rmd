...
# Define variables
housing_variables <- c("Rent", "Socialhousing", "Owned", "Houseprice", "Overoccupation", "lagged_diss")
neighborhood_variables <- c("Crime", "Greenery", "PTAL")
...
# Apply Min-Max Scaling and then divide by a factor
division_factor <- 1000  # Adjust the factor as needed
scaled_segregation <- filtered_segregation %>%
  mutate(across(
    all_of(c(housing_variables, neighborhood_variables)),
    ~ min_max_scaling(.) / division_factor,
    .names = "scaled_{col}"
  ))
...
for (nationality in nationalities) {
  ...
  nationality_data <- scaled_segregation %>%
    filter(Nationality == nationality) %>%
    drop_na(Diss_Index, starts_with("scaled_"))  # Ensure complete cases
  ...
  # 1. Regressions for progressively adding housing variables
  current_formula <- y_variable
  for (i in seq_along(housing_variables)) {
    scaled_vars <- paste0("scaled_", housing_variables[1:i])
    current_formula <- paste(y_variable, "~", paste(scaled_vars, collapse = " + "))
    model <- lm(as.formula(current_formula), data = nationality_data)
    models[[paste0("Housing_Progressive_", paste(scaled_vars, collapse = "_"))]] <- model
  }
  ...
  full_model <- lm(as.formula(full_formula), data = nationality_data)
  models[["FullModel"]] <- full_model
  
  # Save diagnostics plots for the full model
  pdf(file = paste0("regression_outputs/Diagnostics_", nationality, "_FullModel.pdf"))
  par(mfrow = c(2, 2))
  plot(full_model)  # Residuals, QQ, leverage
  dev.off()
  
  # 3. Generate traditional linear regression table
  results_table <- summary(full_model)$coefficients %>%
    as.data.frame() %>%
  ...
  write.csv(results_table, paste0("regression_outputs/RegressionResults_", nationality, ".csv"), row.names = FALSE)
  
  # 4. Generate regression tables in styled formats
  try({
    stargazer(
      models,
      type = "html",
