...
housing_variables <- c("Rent", "budget_proportion", "average_owned", "average_rented", "average_social", "Houseprice")
...
for (nationality in nationalities) {
  ...
  nationality_data <- final_segregation %>% filter(Nationality == nationality)
...
for (nationality in nationalities) {
  ...
  relevant_data <- nationality_data %>%
    ...
    select(c("Rent", "budget_proportion", "average_owned", "average_rented", 
             "average_social", "Houseprice", "Occupation", "Dissimilarity_Index")) %>%
...
# Define nationalities and housing variables
nationalities <- c("Bangladesh", "Caribbean", "Poland", "Romania", "SEAfrica", "SouthAmerica")
housing_variables <- c("Rent", "budget_proportion", "average_owned", "average_rented", "average_social", "Houseprice", "Occupation")
...
# Define variables
nationalities <- c("Bangladesh", "Caribbean", "Poland", "Romania", "SEAfrica", "SouthAmerica")
best_formula <- paste("Dissimilarity_Index ~ Rent + budget_proportion + average_owned + average_rented + average_social + Houseprice + LaggedSettlement + Occupation")

# Ensure directory for GWR outputs
dir.create("gwr_outputs", showWarnings = FALSE)

# Deduplicate geometries
unique_geometries <- final_segregation %>%
...
for (nationality in nationalities) {
  # Filter data for the current nationality
  nat_data <- final_segregation %>%
    filter(Nationality == nationality) %>%
    filter(!is.na(geometry)) %>%  # Ensure spatial features are present
    group_by(MSOA11CD) %>%
    summarize(across(where(is.numeric), mean, na.rm = TRUE), .groups = "drop") %>%
    left_join(unique_geometries, by = "MSOA11CD") %>% # Add unique geometries
  ...
  for (variable in c("Rent", "budget_proportion", "average_owned", "average_rented", "average_social", "Houseprice", "LaggedSettlement")) {
    ggplot(nat_data) +
