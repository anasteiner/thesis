...
tenure_london <- read_csv("/Users/anamariasteinercorrea/Desktop/v3 longitudinal diaspora study/Data/tenure_london.csv")
...
processed_london_price <- price_london %>%
  mutate(
    Period = as.Date(paste0(Period, "-01")),                # Convert year-month to Date
    Year = year(Period),                                    # Extract year from Period
    `Average price All property types` = as.numeric(`Average price All property types`)  # Ensure numeric
  ) %>%
  group_by(Year) %>%
  summarise(
    Houseprice = mean(`Average price All property types`, na.rm = TRUE)  # Average price per year
  ) %>%
  complete(Year = 2001:2021) %>%                            # Fill missing years 2001-2021
  fill(Houseprice, .direction = "downup") %>%              # Fill missing values for interpolation
  filter(!is.na(Houseprice)) %>%                           # Remove rows without house price data
  mutate(
    MSOA11CD = "London",                                   # Add London placeholder
    MSOA11NM = "London",
    geometry = NA                                          # No geometry for London
  ) %>%
  select(MSOA11CD, MSOA11NM, Year, Houseprice, geometry)   # Select relevant columns
...
london_accommodation <- interpolated_accommodation %>%
  st_drop_geometry() %>%               # Temporarily drop geometry for calculations
  group_by(Year) %>%                   # Group by Year to calculate yearly averages
  summarise(
    Occupation = mean(Occupation, na.rm = TRUE)  # Calculate average occupation
  ) %>%
  mutate(
    MSOA11NM = "London",               # Set MSOA name as "London"
    MSOA11CD = "London"                # Set MSOA code as "London"
  )

# Add an empty geometry column for London
london_accommodation$geometry <- st_sfc(
  rep(st_geometrycollection(), nrow(london_accommodation)),  # Add an empty geometry for each row
...
print(accommodation_with_london)
```
...
tenure_london <- tenure_london[-1, ]

# Step 2: Remove all columns that start with "NA_..."
tenure_london <- tenure_london %>%
  select(-starts_with("NA_"))

colnames(tenure_london)

# Step 1: Extract years from "Own Outright" columns
updated_colnames <- colnames(tenure_london)

# Iterate through column names
for (i in seq_along(updated_colnames)) {
  if (str_detect(updated_colnames[i], "Own Outright_\\d{4}")) {
    # Extract the year
    year <- str_extract(updated_colnames[i], "\\d{4}")
    
    # Rename the corresponding columns
    if ((i + 1) <= length(updated_colnames) && str_detect(updated_colnames[i + 1], "Buying with mortgage")) {
      updated_colnames[i + 1] <- paste0("Buying with mortgage_", year)
    }
    if ((i + 2) <= length(updated_colnames) && str_detect(updated_colnames[i + 2], "Rented from Local Authority")) {
      updated_colnames[i + 2] <- paste0("Rented from Local Authority or Housing Association_", year)
    }
    if ((i + 3) <= length(updated_colnames) && str_detect(updated_colnames[i + 3], "Rented from Private landlord")) {
      updated_colnames[i + 3] <- paste0("Rented from Private landlord_", year)
    }
    if ((i + 4) <= length(updated_colnames) && str_detect(updated_colnames[i + 4], "Total_Total")) {
      updated_colnames[i + 4] <- paste0("Total_Total_", year)
    }
  }
}

# Step 2: Assign updated column names back to the data
colnames(tenure_london) <- updated_colnames

# Step 3: Verify the updated column names
print(colnames(tenure_london))

tenure_london_filtered <- tenure_london %>%
  filter(MSOA11NM == "London")

tenure_london_long <- tenure_london_filtered %>%
  pivot_longer(
    cols = matches("_\\d{4}$"),     # Select columns ending with "_Year"
    names_to = c("Category", "Year"), # Extract "Category" and "Year" from column names
    names_pattern = "(.*)_(\\d{4})",  # Regex to split category and year
    values_to = "Value"               # Values go into the "Value" column
  ) %>%
  pivot_wider(                       # Convert to wide format for separate columns
    names_from = Category,           # Create new columns from categories
    values_from = Value              # Values for these columns
  )

tenure_london_final <- tenure_london_long %>%
  mutate(
    average_owned = (`Own Outright` + `Buying with mortgage`) / Total_Total,
    average_rented = `Rented from Private landlord` / Total_Total,
    average_social = `Rented from Local Authority or Housing Association` / Total_Total
  ) %>%
  select(  # Step 2: Keep only the required columns
    MSOA11CD,
    MSOA11NM,
    Year,
    average_owned,
...
tenure_london_final <- tenure_london_final %>%
  mutate(Year = as.integer(Year))  # Convert Year to integer for consistency

# Step 2: Define the complete range of years
all_years <- 2001:2021

# Step 2: Interpolate the missing years for each average column
interpolated_tenure <- tenure_london_final %>%
  group_by(MSOA11CD, MSOA11NM) %>%                # Group by MSOA
  complete(Year = all_years) %>%                  # Ensure all years are present (2001–2021)
  arrange(Year) %>%                               # Sort by year
  mutate(
    # Interpolate each column using approx()
    average_owned = approx(x = Year[!is.na(average_owned)], 
                           y = average_owned[!is.na(average_owned)], 
                           xout = Year, 
                           rule = 2)$y,
    average_rented = approx(x = Year[!is.na(average_rented)], 
                            y = average_rented[!is.na(average_rented)], 
                            xout = Year, 
                            rule = 2)$y,
    average_social = approx(x = Year[!is.na(average_social)], 
...
# Step 1: Add missing fields to the interpolated London data
interpolated_tenure <- interpolated_tenure %>%
  mutate(
    MSOA11CD = "London",   # Set MSOA11CD to "London"
    geometry = NA          # Add an empty geometry column
  )

# Step 2: Rename Year to match `grouped_data`
interpolated_tenure <- interpolated_tenure %>%
  rename(year = Year)  # Rename Year to year for consistency

interpolated_tenure <- interpolated_tenure %>%
  mutate(geometry = st_sfc(st_geometrycollection(), crs = st_crs(grouped_data)))  # Set CRS

# Step 2: Combine the datasets
final_tenancy <- grouped_data %>%
  bind_rows(interpolated_tenure)
```
...
budget_msoa <- budget_lad11cd %>%
  mutate(
    budget_proportion = if_else(
      ...
      Budget,                      # Copy the Budget for London_budget rows
      Budget * Proportion          # Multiply by Proportion for other rows
    )
  )
...
budget_msoa <- budget_msoa %>%
  mutate(
    budget_proportion = if_else(
      LAD11CD == "london_budget",   # Check if the row is for London_budget
      as.numeric(Budget),          # Copy the Budget value for London_budget rows
      as.numeric(Budget) * Proportion # Calculate for other rows
    )
  )
...
# View the fixed dataset
print(head(rents_cleaned))
...
rents_cleaned_annual <- rents_cleaned %>%
  ...
  ungroup() %>%
  select(Code, Area, `2011`:`2019`) # Keep only the summarized years
...
# Step 2: Perform a full join with the MSOA shapefile data
finals_rents <- full_join(msoa, rents_cleaned, by = c("LAD11CD", "LAD11NM"))

finals_rents <- finals_rents %>%
  mutate(
    MSOA11CD = ifelse(LAD11CD == "E12000007", "London", MSOA11CD),
    MSOA11NM = ifelse(LAD11CD == "E12000007", "London", MSOA11NM)
  )

finals_rents <- finals_rents %>%
  rowwise() %>%
  mutate(
    `2011` = sum(c_across(starts_with("numeric_2011 Q")), na.rm = TRUE),
    `2012` = sum(c_across(starts_with("numeric_2012 Q")), na.rm = TRUE),
    `2013` = sum(c_across(starts_with("numeric_2013 Q")), na.rm = TRUE),
    `2014` = sum(c_across(starts_with("numeric_2014 Q")), na.rm = TRUE),
    `2015` = sum(c_across(starts_with("numeric_2015 Q")), na.rm = TRUE),
    `2016` = sum(c_across(starts_with("numeric_2016 Q")), na.rm = TRUE),
    `2017` = sum(c_across(starts_with("numeric_2017 Q")), na.rm = TRUE),
    `2018` = sum(c_across(starts_with("numeric_2018 Q")), na.rm = TRUE),
    `2019` = sum(c_across(starts_with("numeric_2019 Q")), na.rm = TRUE)
  ) %>%
  ungroup()

final_rents <- finals_rents %>%
  select(MSOA11CD, MSOA11NM, geometry, Proportion, `2011`:`2019`)

final_rents <- final_rents %>%
  mutate(across(`2011`:`2019`, ~ ifelse(MSOA11NM != "London", .x * Proportion, .x))) %>%
  select(-Proportion)

final_rents_long <- final_rents %>%
  pivot_longer(
    cols = `2011`:`2019`,
    names_to = "Year",
    values_to = "Rent"
  ) %>%
  mutate(Year = as.numeric(Year))  # Ensure Year is numeric for interpolation

# Define the full range of years for interpolation
full_years <- seq(2001, 2021)

# Interpolate missing years for each MSOA11CD
interpolated_rents <- final_rents_long %>%
  group_by(MSOA11CD, MSOA11NM, geometry) %>%
  complete(Year = full_years) %>%  # Add missing years
  arrange(Year) %>%
  mutate(
    Rent = zoo::na.approx(Rent, x = Year, na.rm = FALSE),  # Linear interpolation
    Rent = if_else(is.na(Rent), zoo::na.locf(Rent, na.rm = FALSE, fromLast = FALSE), Rent),  # Fill leading NAs
    Rent = if_else(is.na(Rent), zoo::na.locf(Rent, na.rm = FALSE, fromLast = TRUE), Rent)   # Fill trailing NAs
  ) %>%
  ungroup()
```
...
for (policy_name in names(policy)) {
  ...
  msoa_with_years[[policy_name]] <- with(msoa_with_years,
    ifelse(
...
finals_dummies <- msoa_with_years %>%
  select(MSOA11CD, MSOA11NM, Year, geometry, Right_to_Buy, Welfare_Reform_Act, Elephant_and_Castle_Redevelopment, Affordable_Housing_Program, Housing_Act_2004)
```

# Final housing data
```{r}
# Step 1: Filter datasets for odd years
datasets <- list(
  interpolated_rents,
  final_budget,
  final_tenancy,
...
filtered_datasets <- lapply(datasets, function(data) {
  data %>% filter(Year %% 2 != 0) # Keep only odd years
})
...
# Standardize CRS if not already aligned
target_crs <- st_crs(final_dummies) # Use CRS of the first dataset
standardized_datasets <- lapply(standardized_datasets, function(data) {
  if (st_crs(data) != target_crs) {
    st_transform(data, target_crs)
  } else {
    data
  }
})
...
# Step 3: Full join all datasets
joined_data <- interpolated_rents %>%
  st_join(final_budget, join = st_intersects) %>%  # Spatial join based on geometries
  st_join(final_tenancy, join = st_intersects) %>%
  st_join(combined_houseprice, join = st_intersects) %>%
  st_join(accommodation_with_london, join = st_intersects)
...
housing_no_geo$geometry <- NULL


# Write the cleaned data frame to a CSV file
write.csv(housing_no_geo, "housing_data.csv", row.names = FALSE)
```
