...
private_rents <- read_csv("/Users/anamariasteinercorrea/Desktop/v3 longitudinal diaspora study/Data/private rents.csv")
...
updated_rents <- private_rents %>%
  mutate(LAD11CD = if_else(row_number() == 41, "London", LAD11CD))
...
rents_geom <- rents_filtered %>%
  full_join(msoa, by = "LAD11CD")

rents_geom <- rents_geom %>%
  mutate(
    MSOA11CD = if_else(LAD11CD == "London", "London", MSOA11CD), # Rename MSOA11CD for London_budget
    MSOA11NM = if_else(LAD11CD == "London", "London", MSOA11NM)  # Rename MSOA11NM for London_budget
  )

# Select the desired columns
final_rents <- rents_geom %>%
...
msoa <- st_read("/path/to/MSOA_2011_London_gen_MHW.shp")

# Define policy-specific boroughs and years
policies <- list(
  "Right_to_Buy" = list(
    boroughs = c("Barking", "Havering", "Redbridge", "Newham", "Haringey"),
    years = list(pre = 0, during = 1, post = 0),
    year_ranges = c("Pre-1980", "1980–2010", "Post-2010")
  ),
  "Welfare_Reform_Act" = list(
    boroughs = c("Newham", "Tower Hamlets", "Barking and Dagenham", "Hackney", "Enfield"),
    years = list(pre = 0, during = 1, post = 0),
    year_ranges = c("Pre-2012", "2012–2016", "Post-2016")
  ),
  "Elephant_and_Castle_Redevelopment" = list(
    boroughs = c("Southwark"),
    years = list(pre = 0, during = 1, post = 0),
    year_ranges = c("Pre-2004", "2004–2021", "Post-2021")
  ),
  "Affordable_Housing_Program" = list(
    boroughs = c("Newham", "Tower Hamlets", "Croydon", "Barking"),
    years = list(pre = 0, during = 1, post = 0),
    year_ranges = c("Pre-2016", "2016–2022", "Post-2022")
  ),
  "Housing_Act_2004" = list(
    boroughs = c("Tower Hamlets", "Hackney", "Newham", "Brent", "Ealing"),
    years = list(pre = 0, during = 1, post = 0),
    year_ranges = c("Pre-2004", "2004–2011", "Post-2011")
  )
)

# Initialize the policy_dummies dataframe
policy_dummies <- msoa %>%
  dplyr::select(MSOA11CD, MSOA11NM, geometry)

# Create dummy columns for each policy
for (policy in names(policies)) {
  policy_info <- policies[[policy]]
  policy_boroughs <- policy_info$boroughs
  
  # Assign 1 to MSOAs in the specified boroughs, 0 otherwise
  policy_dummies <- policy_dummies %>%
    mutate(!!policy := ifelse(MSOA11NM %in% policy_boroughs, 1, 0))
}

# Write the resulting dataset to a CSV or GeoPackage
st_write(policy_dummies, "policy_dummies.gpkg", layer = "policy_dummies", driver = "GPKG")

# Save as a CSV file if geometry is not needed
st_drop_geometry(policy_dummies) %>%
  write.csv("policy_dummies.csv", row.names = FALSE)
```
