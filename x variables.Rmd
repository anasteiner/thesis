...
tenure_FINAL <- tenure_FINAL %>%
  mutate(across(
    -c(MSOA11CD, MSOA11NM), # Skip the first two columns
    ~ as.numeric(gsub("[^0-9.]", "", .)), # Remove non-numeric characters and convert to numeric
    .names = "cleaned_{col}"
  ))

# Step 2: Convert to long format with a Year column
tenure_LONG <- tenure_FINAL %>%
  pivot_longer(
    cols = starts_with("20"), # Select columns starting with years
    names_to = "Category_Year", # Create one column for year and category
    values_to = "Value" # Place values here
  )

# Step 3: Calculate averages
tenure_FINAL_long <- tenure_FINAL_long %>%
  mutate(
    average_owned = (`Own Outright` + `Buying with mortgage`) / Total,
    average_rented = `Rented from Private landlord` / Total,
    average_social = `Rented from Local Authority or Housing Association` / Total
  )

# Step 4: Filter to keep only row 34 (London information)
tenure_FINAL_long_filtered <- tenure_FINAL_long %>%
  filter(MSOA11CD == "E12000007")

# Step 5: Keep only necessary columns
tenure_FINAL_long_filtered <- tenure_FINAL_long_filtered %>%
  select(MSOA11CD, MSOA11NM, Year, average_owned, average_rented, average_social)
...
finals_rents <- finals_rents  %>%
  select(MSOA11CD, MSOA11NM, geometry, Proportion, "2011", "2012", "2013", "2014", "2015", "2016", "2017", "2018", "2019")

rents_long <- finals_rents %>%
  pivot_longer(
    cols = starts_with("201"),  # Columns representing years
    names_to = "Year",          # New column for years
    values_to = "Rent"          # New column for rent values
  ) %>%
  select(MSOA11CD, MSOA11NM, geometry, Proportion, Year, Rent)

rents_long <- rents_long %>%
  mutate(
    Rent = ifelse(
      MSOA11CD == "London",
      Rent,                             # Keep Rent unchanged for "London"
      Rent * Proportion                 # Multiply Rent by Proportion for others
    )
  )
...
interpolated_rents <- rents_long %>%
  group_by(MSOA11CD, MSOA11NM) %>%
  complete(Year = all_years) %>%  # Ensure all years are present
  arrange(Year) %>%
  mutate(
    Rent = if (sum(!is.na(Rent)) > 1) {
      ...
      Rent  # Keep Rent unchanged if not enough data for interpolation
    }
  ) %>%
  ungroup()
...
cleaned_housing <- cleaned_housing %>%
  arrange(ID, Year) %>% # Ensure data is sorted
