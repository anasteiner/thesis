...
msoa_census <- msoa_geometry %>%
    left_join(finalest_census_dt, by = "MSOA11NM", "MSOA11CD")

# Clean final columns
msoa_census <- msoa_census[, c("MSOA11CD.x", "MSOA11NM", "LAD11NM", "Year", "Nationality", "Proportion", "Dissimilarity_Index", "lagged_diss", "H", "Isolation_Index", "geometry")]
```

```{r}
# Calculate statistics
min_value <- min(msoa_census$Dissimilarity_Index, na.rm = TRUE)
max_value <- max(msoa_census$Dissimilarity_Index, na.rm = TRUE)
median_value <- median(msoa_census$Dissimilarity_Index, na.rm = TRUE)
mean_value <- mean(msoa_census$Dissimilarity_Index, na.rm = TRUE)
