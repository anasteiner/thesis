...
# Convert dataset to sf object and set CRS (replace 27700 with your dataset's CRS if different)
...
general_gwr_data <- scaled_segregation_sf %>%
  ...
  summarize(
    ...
    geometry = first(geometry)  # Keep one geometry per MSOA
  ) %>%
...
# Replace 'residuals' with the variable you wish to analyze (e.g., gwr_coeff_mean_rent)
merged_gwr$residuals <- merged_gwr$gwr_fitted - merged_gwr$mean_diss  # Example residuals
...
residuals_plot <- ggplot(merged_gwr) +
  geom_sf(aes(fill = residuals), color = NA) +  # Use the residuals column for coloring
  scale_fill_gradient2(
    low = "blue", mid = "white", high = "red", midpoint = 0,
    name = "Residuals"
  ) +
...
# Ensure scaled_segregation_sf is an sf object
scaled_segregation_sf <- scaled_segregation %>%
  st_as_sf(wkt = "geometry", crs = 27700)
...
```{r}
# Create a directory for trends over time
...
for (var in housing_vars) {
  ...
  yearly_data <- scaled_segregation %>%
    group_by(Year) %>%
  ...
  plot <- ggplot(yearly_data, aes(x = Year, y = mean_value)) +
    ...
    labs(title = paste("Temporal Trend of", var),
...
# Ensure dataset is an sf object
scaled_segregation_sf <- st_as_sf(scaled_segregation, wkt = "geometry", crs = 27700)

# Create a directory for spatial maps
if (!dir.exists("plots/maps")) dir.create("plots/maps", recursive = TRUE)

# Loop through each variable and generate a spatial map
for (var in housing_vars) {
  plot <- ggplot(scaled_segregation_sf) +
    geom_sf(aes(fill = .data[[var]]), color = NA) +
    scale_fill_gradient(low = "blue", high = "red", name = var) +
    labs(title = paste("Spatial Distribution of", var)) +
    theme_minimal()
  
  # Save the plot
  ggsave(filename = paste0("plots/maps/", var, "_Map.png"), plot = plot, width = 10, height = 8)
  
  print(paste("Spatial map for", var, "completed."))
}
```
