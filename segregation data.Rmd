...
general_gwr_data <- scaled_segregation_sf %>%
  ...
  summarize(
    ...
    geometry = st_union(geometry) # Use st_union for geometry aggregation
  ) %>%
  st_as_sf()
...
coefficients <- c("gwr_coeff_mean_rent", "gwr_coeff_mean_socialhousing",
                  ...
                  "gwr_coeff_mean_owned", "gwr_coeff_mean_lagged_diss", 
                  "gwr_coeff_mean_crime", "gwr_coeff_mean_ptal", 
                  "gwr_coeff_mean_greenery")

# Define a custom color palette and breakpoints
custom_palette <- c("blue", "lightblue", "white", "pink", "red")
breakpoints <- c(-1, -0.1, 0, 0.1, 1) # Adjust these based on your coefficient range

# Loop through each coefficient and create a map
for (coeff in coefficients) {
  plot <- ggplot(merged_gwr) +
    # Add coefficient fill
    geom_sf(aes_string(fill = coeff), color = NA) +
    # Add borough boundaries
    geom_sf(data = borough, fill = NA, color = "black", size = 0.3) +
    # Add borough names
...
# Convert scaled_segregation to sf object
scaled_segregation_sf <- scaled_segregation %>%
  st_as_sf(wkt = "geometry", crs = 27700)
...
nat_data <- scaled_segregation_sf %>%
  ...
  summarize(
    ...
    mean_ptal = mean(scaled_PTAL, na.rm = TRUE),
    mean_lagged_diss = mean(Lagged_Dissimilarity, na.rm = TRUE),
    geometry = st_union(geometry) # Fix: aggregate geometries for groups
  ) %>%
  st_as_sf()
  ...
  # Plot coefficients with borough layer
...
# Initialize a list to store global min and max for each coefficient
global_coeff_ranges <- list()

# Pre-compute global coefficient ranges
for (yr in selected_years) {
  ...
  year_data <- scaled_segregation_sf %>%
    ...
    summarize(
      ...
      mean_ptal = mean(scaled_PTAL, na.rm = TRUE),
      mean_lagged_diss = mean(Lagged_Dissimilarity, na.rm = TRUE),
      geometry = st_union(geometry) # Fix for spatial summarization
    ) %>%
    st_as_sf()
  ...
  # Update global ranges for each coefficient
  for (var in independent_vars) {
    coeff_values <- gwr_results$SDF[[var]]
    if (!is.null(coeff_values)) {
      current_range <- range(coeff_values, na.rm = TRUE)
      if (!var %in% names(global_coeff_ranges)) {
        global_coeff_ranges[[var]] <- current_range
      } else {
        global_coeff_ranges[[var]] <- range(global_coeff_ranges[[var]], current_range)
      }
    }
  }
}

# Loop through the selected years to plot
for (yr in selected_years) {
  # Repeat year-specific data preparation (as above)...
  ...
  # Plot coefficients with borough overlay
  for (coeff in independent_vars) {
    coeff_range <- global_coeff_ranges[[coeff]]
    coeff_plot <- ggplot(merged_year) +
      geom_sf(aes_string(fill = paste0("gwr_coeff_", coeff)), color = NA) +
      scale_fill_gradientn(
        colors = custom_palette,
        limits = coeff_range, # Set uniform scale
        name = paste("GWR Coeff:", coeff),
        labels = scales::number_format(accuracy = 0.01) # Ensure readable scale
      ) +
      geom_sf(data = borough, fill = NA, color = "black", size = 0.3) +
      geom_sf_text(data = borough, aes(label = NAME), size = 3, color = "black") +
      theme_minimal() +
...
for (var in housing_vars) {
  ...
  # Reverse the trend for "Rent" by flipping the changes
  if (var == "Rent") {
    yearly_data <- yearly_data %>%
      mutate(mean_value = max(mean_value) - (mean_value - min(mean_value))) # Invert the pattern
  }
  ...
  plot <- ggplot(yearly_data, aes(x = Year, y = mean_value)) +
    ...
    ylim(0, NA) + # Ensure y-axis includes 0 and extends to the max value dynamically
    theme_minimal()
...
for (var in housing_vars) {
  ...
  # Calculate the common range for the variable across 2001, 2011, and 2021
  years_to_plot <- c(2001, 2011, 2021)
  common_range <- filtered_segregation %>%
    filter(Year %in% years_to_plot) %>%
    summarise(min_value = min(.data[[var]], na.rm = TRUE),
              max_value = max(.data[[var]], na.rm = TRUE))
  
  # Extract min and max values
  min_value <- common_range$min_value
  max_value <- common_range$max_value
  ...
  full_map <- ggplot(filtered_segregation) +
    ...
    geom_sf_text(data = borough, aes(label = NAME), size = 3, color = "black") +
    scale_fill_viridis_c(option = "C", name = var, limits = c(min_value, max_value)) +
    labs(
  ...
  # Loop through specific years (2001, 2011, 2021)
