...
```{r}
# Filter MSOAs with Dissimilarity_Index > 1 for each nationality
high_dissimilarity_msoas <- finalest_census_dt %>%
  filter(Dissimilarity_Index > 1) %>% # Filter rows where the index is above 1
  group_by(Nationality) %>% # Group by nationality
  summarise(
    MSOA_Count = n(), # Count the number of MSOAs
    MSOAs = paste(unique(MSOA11CD), collapse = ", ") # List unique MSOA codes
  ) %>%
  arrange(desc(MSOA_Count)) # Arrange by number of MSOAs in descending order
...
min_value <- min(finalest_census_dt$Dissimilarity_Index, na.rm = TRUE)
max_value <- max(finalest_census_dt$Dissimilarity_Index, na.rm = TRUE)
median_value <- median(finalest_census_dt$Dissimilarity_Index, na.rm = TRUE)
mean_value <- mean(finalest_census_dt$Dissimilarity_Index, na.rm = TRUE)
...
```{r}
borough_stats <- finalest_census_dt %>%
  group_by(LAD11NM, Nationality) %>%
  summarize(
    mean_Dissimilarity_Index = mean(Dissimilarity_Index, na.rm = TRUE),
    min_Dissimilarity_Index = min(Dissimilarity_Index, na.rm = TRUE),
    max_Dissimilarity_Index = max(Dissimilarity_Index, na.rm = TRUE),
    .groups = "drop"
  )
...
change_stats <- finalest_census_dt %>%
  ...
  group_by(MSOA11CD, LAD11NM, Nationality) %>%
  summarize(
