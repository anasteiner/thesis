...
dissimilarity_plot <- ggplot(aggregated_data, aes(x = Year, y = D_Index, color = Nationality, group = Nationality)) +
  ...
  geom_point(size = 2) +
  scale_y_continuous(
    breaks = scales::pretty_breaks(n = 6), # Automatically generate breaks for better readability
  ...
  labs(
    ...
    x = "Year",
    y = "Dissimilarity Index", # Add clear y-axis label
...
change_stats <- census_data_clean %>%
  filter(!is.na(D_Index), !is.na(Nationality)) %>%  # Remove rows with NA Diss_Index or Nationality
  group_by(MSOA11CD, Nationality) %>%
  summarize(
    Diss_Index_Change = max(D_Index) - min(D_Index),  # No need for na.rm as NA rows are excluded
    .groups = "drop"
  )
...
for (nationality in unique(filtered_segregation$Nationality)) {
  ...
  correlation_data <- nat_data %>%
    dplyr::select(
      D_Index, Proportion, Rent, Socialhousing, Owned, Houseprice,
...
for (nationality in unique(scaled_segregation$Nationality)) {
  ...
  models <- list(
    ...
    lm(D_Index ~ scaled_Rent + scaled_Socialhousing + scaled_Houseprice + scaled_Overoccupation + scaled_Owned + Lagged_Dissimilarity + scaled_Crime + scaled_Greenery + scaled_PTAL, data = nationality_data)
  )
  ...
  stargazer(
    ...
    covariate.labels = c(
      "Rent", "Social Housing", "House Price", "Overoccupation",
      "Owned", "Lagged Dissimilarity", "Crime", "Greenery", "PTAL"
    ),
